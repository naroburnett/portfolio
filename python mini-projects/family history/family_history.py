...
def print_death_age(people_dict):
    ...
    for key, value in people_dict.items():
        ...
        age = death - birth
        print(f"{name}, was born in the year {birth}. Lived a good life and then died at the age of {age} years old, during the year of {death}.")
        print()
...
def count_genders(people_dict):
    ...
    for key, value in people_dict.items():
        if value[GENDER_INDEX] == "M":
            ...
            female += 1
    print (f"There is {male} male(s) in the system.")
    print (f"There is {female} female(s) in the system.")
...
def print_marriages(marriages_dict, people_dict):
    ...
    for key, value in marriages_dict.items():
        Husband_Index = value[0]
        Wife_Index = value[1]
        Wed_Year = value[2]
        value = people_dict[Husband_Index]
 
        Husband = value[NAME_INDEX]
        Husband_Birth = value[BIRTH_YEAR_INDEX]
        Husband_Age = Wed_Year - Husband_Birth
 
        value = people_dict[Wife_Index]
 
        Wife = value[NAME_INDEX]
        Wife_Birth = value[BIRTH_YEAR_INDEX]
        Wife_Age = Wed_Year - Wife_Birth
 
        print()
        print(f"{Husband}: {Husband_Age}")
        print(f"In {Wed_Year} Married")
        print(f"{Wife}: {Wife_Age}")
        print()

def count_marriages(marriages_dict, people_dict):
    #Make a loop to count instances of 1 person then make a if that if that person pops up again skip
    for item in marriages_dict.items():
        key = item[0]
        value = [1]
        mc = 0
        if value[]
        print(f"{Husband} was married {marriage_count} time(s)")
